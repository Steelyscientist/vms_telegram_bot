...
from keyboards.default.Themes import app_theme_ru, app_theme_uz
...
async def bot_start(message: types.Message, state: FSMContext):
    ...
    modul = message.text
    username = message.from_user.username
    langugae = db.select_user(id=user_id)
    print(langugae)
    if username:
        user_info = f"@{username}"
    else:
    ...
    await message.answer(
        "Выберите тему обращения или введите свою.", reply_markup=application_page_ru
    )
    await appstate.type_app.set()

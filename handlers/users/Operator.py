...
@dp.message_handler(text="/allusers", user_id=OPERATOR)
async def get_all_users(message: types.Message):
    users = db.select_all_users()
    print(users[0][0])
    await message.answer(users)

@dp.message_handler(text="/post", user_id=OPERATOR)
...
@dp.message_handler(text="Ortga / Назад", user_id=OPERATOR)
async def send_ad_to_all(message: types.Message):
    await message.answer("Bo'limni tanlang / Выберите раздел", reply_markup=start)

@dp.message_handler(text="Javob berish / Отвечать", user_id=OPERATOR)
async def send_ad_to_all(message: types.Message):
    await message.answer("Matnini kiriting / Введите текст", reply_markup=cancle_btn)
    await adminstate.ansver.set()

@dp.message_handler(text="Bekor qilish / Отмена", user_id=OPERATOR, state = adminstate.ansver)
...
@dp.message_handler(content_types=types.ContentType.ANY, user_id=OPERATOR, state = adminstate.ansver)
...
@dp.message_handler(text="Bekor qilish / Отмена", user_id=OPERATOR, state = adminstate.userid)
async def bot_start(message: types.Message, state: FSMContext):
    await message.answer("Bo'limni tanlang / Выберите раздел", reply_markup=admin_uz)
    await state.finish()
...
@dp.message_handler(content_types=types.ContentType.ANY, user_id=OPERATOR, state = adminstate.userid)
async def bot_start(message: types.Message, state: FSMContext):
    if message.content_type == 'text':
        if is_numbers_only(message.text):
            ...
            if userid:
                ...
                await message.answer("Javobni yuborishini tasdiqlash uchun <b>Tasdqilayman</b> jumlasini yuboring \n\n Отправьте <b>Подтверждаю</b>,чтобы подтвердить отправку ответа.", reply_markup=cancle_btn)
...
@dp.message_handler(text="Bekor qilish / Отмена", user_id=OPERATOR, state = adminstate.confirm)
async def bot_start(message: types.Message, state: FSMContext):
    await message.answer("Bo'limni tanlang / Выберите раздел", reply_markup=admin_uz)
    await state.finish()
def is_confirm_only(text):
    return all("Tasdqilayman" or "Подтверждаю" in text)

@dp.message_handler(content_types=types.ContentType.ANY, user_id=OPERATOR, state = adminstate.confirm)
async def bot_start(message: types.Message, state: FSMContext):
    if message.content_type == 'text':
...
@dp.message_handler(text="/cleandb", user_id=OPERATOR,)
async def get_all_users(message: types.Message):
    db.delete_users()
